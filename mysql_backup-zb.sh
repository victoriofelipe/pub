...
#                Date: 21/06/2007
#
# Modified By:   Victorio H Felipe
#                Date: 01/06/2011
#
#
#
logger -t backup_sys "mysql backup start"
# NOW
DT=$(date +%Y-%m-%d__%H-%M)
...
for DB in $DBS_LIST; do
    DB_BKP_FLDR=$BASE_BAK_FLDR/$DT/$DB
    ...
    mysqldump -u $DB_USER $DB_PASS -B -R -d --single-transaction $DB | \
            gzip -c > $DB_BKP_FLDR/000-$DB-SCHEMA.sql.gz
    index=0
    ...
    while [ "$index" -lt "$table_type_count" ]; do
        ...
        if [ $table == "history" ] || [ $table == "history_uint" ]; then continue; fi
...
logger -t backup_sys "mysql backup end"
